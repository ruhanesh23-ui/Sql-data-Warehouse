...
CREATE TABLE silver.crm_prd_info (
    ...
    prd_cost        Int,   -- Raw product cost — cast to DECIMAL in Silver
    prd_line        NVARCHAR(50),   -- Product line or category (e.g. 'Road', 'Mountain')
    prd_start_dt    Date,   -- Raw product availability start date — cast to DATE in Silver
    prd_end_dt      Date,    -- Raw product availability end date — cast to DATE in Silver
    dwh_create_date  datetime2 Default Getdate()                                -- Stored as NVARCHAR because source CSV contains NULLs
...
CREATE TABLE silver.crm_sales_details (
    ...
    sls_order_dt    Date,            -- Order date stored as integer in source (e.g. 20260325) — cast to DATE in Silver
    sls_ship_dt     Date,            -- Shipment date stored as integer in source — cast to DATE in Silver
    sls_due_dt      Date,            -- Payment due date stored as integer in source — cast to DATE in Silver
